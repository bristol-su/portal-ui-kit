        ...
        <div v-if="getTabs().length > 1">
          <div class="group p-0.5 rounded-lg flex bg-gray-100 hover:bg-gray-200">
            ...
            >
              ...
              <span class="sr-only lg:not-sr-only text-gray-600 group-hover:text-gray-900" :class="selectedIndex === index ? 'text-gray-900' : 'text-gray-600 group-hover:text-gray-900'">{{ title }}</span>
            </button>
          </div>
        </div>
        <div v-else>
          <h2 class="mt-12 mb-2 text-2xl font-semibold tracking-tighter text-black sm:text-3xl title-font" v-for="title in getTabs()">{{ title }}</h2>
        </div>
      </div>
    </header>
    <slot></slot>
